...
# Extracting data from content.json
...
user_context = f"\n\ninstructions: {instructions}\n\nRestrictions: {restrictions}" + " With all these information try to negotiate with the customer to make them provide their flat requirement specifications and to try to negotiate with the customer to change their budget if it is a little bit low and then book a site visit anyway and you should be giving some great rebuttals if the customer disagrees"
...
# Function to check the qualification status of the customer
...
@app.route('/')
def index():
    return render_template('index.html')  # Render your HTML interface
...
def submit():
    ...
    response = CustomChatGPT(phone_number, user_input)  # Call your function
